...
func TestQueuedRunPersistsBeforeExecutionAndKeepsCreationTime(t *testing.T) {
	...
	runner, err := engine.New(1, 1, func(ctx context.Context, step workflow.Step) (workflow.CheckResult, error) {
		...
		if saved.Status != "running" || saved.StartedAt.IsZero() || saved.Steps[0].Status != "running" {
			return workflow.CheckResult{}, fmt.Errorf("execution began before promotion was saved: %+v", saved)
		}
		if step.ID == "a" {
			select {
			case <-release:
