...
func TestSaveFailureStopsLaterStepsWithoutReplayingActions(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if !reflect.DeepEqual(lastAttempt, finished) || finished.FinishedAt == nil || runner.active {
...
func TestShutdownSavesCancellationAndWaitsForFinalWrite(t *testing.T) {
	for _, phase := range []string{"saving", "executing"} {
		t.Run(phase, func(t *testing.T) {
			...
			var saved Run
			executed := 0
			runner := New(func(ctx context.Context, _ workflow.Step) (workflow.HTTPResult, error) {
				executed++
				close(begun)
				<-ctx.Done()
